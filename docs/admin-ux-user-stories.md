...
| B6 | Cancel remove keeps the admin | Remove → Cancel | — | **add** |
| B7 | Viewing **my own** membership hides Remove | Open own detail as allowlisted admin | — | **add** |
| B8 | Invalid email shows field error | Create with `nope` | — | **add** |
| B9 | Duplicate email shows Conflict | Create same email twice | — | **add** |
| B10 | Adding owner email shows Conflict | Create `owner@example.edu` | — | **add** |
| B11 | Missing member id → Not found | `/admin/admin-users/missing` | — | **add** |
...
| C3 | Sort by Name ascending | Click Name once | — | **add** |
| C4 | Open edit from list row click | Click first row | — | **add** |
| C5 | Create / validate integer / edit / delete cancel+confirm | Full CRUD | `E2E10-E2E13…` | keep |
| C6 | Required field validation | Create with empty name | — | **add** |
| C7 | Empty list shows No records | Clear Product seed | — | **add** |
| C8 | Entity Forbidden on list | `setForbidden(["Product"])` | `E2E14…` | keep |
| C9 | Entity Forbidden on create | Forbidden + open `/new` | — | **add** |
...
| D2 | List shows the created row | After create, visit list | — | **add** |
| D3 | Edit + Save persists | Change title → Save → reload | — | **add** |
| D4 | Delete cancel then confirm | Same pattern as Products | — | **add** |
...
| E3 | Unknown resource `/new` → Not found | — | **add** |
